...
class WgslParser {
  ...
  _statement() {
    ...
    if (this._check(TokenTypes.keywords.switch)) return this._switch_statement();
  ...
  _variable_statement() {
    if (this._check(TokenTypes.keywords.var)) {
      ...
      if (_var === null) throw this._error(this._peek(), "Variable declaration expected.");
      let value = null;
      if (this._match(TokenTypes.tokens.equal)) value = this._short_circuit_or_expression();
  ...
  _loop_statement() {
    ...
    let continuing = null;
    if (this._match(TokenTypes.keywords.continuing)) continuing = this._compound_statement();
  ...
  _case_selectors() {
    var _a, _b, _c, _d;
    const selectors = [(_b = (_a = this._shift_expression()) === null || _a === void 0 ? void 0 : _a.evaluate(this._context).toString()) !== null && _b !== void 0 ? _b : ""];
    while (this._match(TokenTypes.tokens.comma)) {
  ...
  _if_statement() {
    ...
    let _else = null;
    if (this._match(TokenTypes.keywords.else)) _else = this._compound_statement();
  ...
  _elseif_statement() {
    let elseif = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    const condition = this._optional_paren_expression();
    const block = this._compound_statement();
  ...
  _struct_decl() {
    ...
    while (!this._check(TokenTypes.tokens.brace_right)) {
      ...
      if (!this._check(TokenTypes.tokens.brace_right)) this._consume(TokenTypes.tokens.comma, "Expected ',' for struct member.");else this._match(TokenTypes.tokens.comma);
  ...
  _global_variable_decl() {
    const _var = this._variable_decl();
    if (_var && this._match(TokenTypes.tokens.equal)) _var.value = this._const_expression();
    return _var;
  }
  _override_variable_decl() {
    const _override = this._override_decl();
    if (_override && this._match(TokenTypes.tokens.equal)) _override.value = this._const_expression();
    return _override;
  }
  _global_const_decl() {
  ...
  _const_expression() {
    if (this._match(TokenTypes.const_literal)) return new StringExpr(this._previous().toString());
    const type = this._type_decl();
    this._consume(TokenTypes.tokens.paren_left, "Expected '('.");
    let args = [];
  ...
  _variable_decl() {
    ...
    if (this._match(TokenTypes.tokens.less_than)) {
      ...
      if (this._match(TokenTypes.tokens.comma)) access = this._consume(TokenTypes.access_mode, "Expected access_mode.").toString();
      this._consume(TokenTypes.tokens.greater_than, "Expected '>'.");
    }
    const name = this._consume(TokenTypes.tokens.ident, "Expected variable name");
    let type = null;
  ...
  _type_decl() {
    ...
    if (this._check(TokenTypes.template_types)) {
      ...
      if (this._match(TokenTypes.tokens.less_than)) {
        ...
        access = null;
        if (this._match(TokenTypes.tokens.comma)) access = this._consume(TokenTypes.access_mode, "Expected access_mode for pointer").toString();
  ...
  _texture_sampler_types() {
    if (this._match(TokenTypes.sampler_type)) return new SamplerType(this._previous().toString(), null, null);
    if (this._match(TokenTypes.depth_texture_type)) return new SamplerType(this._previous().toString(), null, null);
...
class FunctionInfo {
  constructor(name) {
    let stage = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    this.stage = null;
    this.inputs = [];
    this.outputs = [];
    this.name = name;
...
class WgslReflect {
  ...
  _getOutputs(type) {
    let outputs = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : undefined;
  ...
  _getInputs(args) {
    let inputs = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : undefined;
