...
function getFramebuffer(device, useFloatTarget) {
    ...
    return device.createFramebuffer({
      ...
      colorAttachments: [
        device.createTexture({
          ...
          type: import_constants.GL.FLOAT,
          mipmaps: false
        })
      ]
    });
  ...
  return device.createFramebuffer({
    width: 256,
    height: 64,
    colorAttachments: [device.createTexture({ format: "rgba8unorm", type: import_constants.GL.FLOAT, mipmaps: false })]
  });
}
function getModel(device, shaderOptions, useFloatTarget) {
  ...
  return new import_engine.Model(device, {
    ...
    vertexCount: 1,
    isInstanced: false,
    drawMode: import_constants.GL.POINTS,
    vs: AGGREGATE_VS,
    fs: AGGREGATE_FS,
    ...shaderOptions
  });
...
var parameters = {
  blend: true,
  blendFunc: [import_constants.GL.ONE, import_constants.GL.ONE, import_constants.GL.ONE, import_constants.GL.ONE],
  blendEquation: [import_constants.GL.FUNC_ADD, import_constants.GL.FUNC_ADD],
  depthTest: false
};
  ...
  initializeState(context, extension) {
    ...
    if (attributeManager) {
      attributeManager.add({
        filterValues: {
          size: filterSize,
          type: fp642 ? import_constants2.GL.DOUBLE : import_constants2.GL.FLOAT,
        ...
        filterCategoryValues: {
          size: categorySize,
          type: import_constants2.GL.FLOAT,
...
// dist/fill-style/shaders.glsl.js
...
var MaskPass = class extends import_core13._LayersPass {
  ...
  render(options) {
    ...
    return this.device.withParametersWebGL({
      blend: true,
      blendFunc: [import_constants3.GL.ZERO, import_constants3.GL.ONE],
      blendEquation: import_constants3.GL.FUNC_SUBTRACT,
      colorMask,
      depthTest: false
...
function makeViewport(opts) {
  ...
  return isGeospatial ? new import_core14.WebMercatorViewport({
    id: viewport.id,
    x: border,
    y: border,
    width,
    ...
    zoom,
    orthographic: true
  }) : new import_core14.OrthographicViewport({
    id: viewport.id,
    x: border,
    y: border,
    width,
...
var MaskEffect = class {
  ...
  preRender(device, { layers, layerFilter, viewports, onViewportActive, views, isPicking }) {
    ...
    if (viewport.resolution !== void 0) {
      import_core15.log.warn("MaskExtension is not supported in GlobeView")();
      return { didRender };
    }
  ...
  _renderChannel(channelInfo, { layerFilter, onViewportActive, views, viewport, viewportChanged }) {
    ...
    if (maskChanged || viewportChanged) {
      ...
      if (maskChanged || !(0, import_core16.equals)(channelInfo.bounds, oldChannelInfo.bounds)) {
  ...
  _sortMaskChannels(maskLayers) {
    ...
    for (const layer of maskLayers) {
      ...
      if (!channelInfo) {
        if (++channelCount > 4) {
          import_core15.log.warn("Too many mask layers. The max supported is 4")();
          continue;
        }
        channelInfo = {
          id,
...
var CollisionFilterEffect = class {
  ...
  preRender(device, { effects: allEffects, layers, layerFilter, viewports, onViewportActive, views, isPicking, preRenderStats = {} }) {
    ...
    const effects = allEffects == null ? void 0 : allEffects.filter((e) => e.constructor === MaskEffect);
  ...
  _render(renderInfo, { effects, layerFilter, onViewportActive, views, viewport, viewportChanged }) {
    ...
    const needsRender = viewportChanged || renderInfo === oldRenderInfo || !(0, import_core18._deepEqual)(oldRenderInfo.layers, renderInfo.layers, 1) || renderInfo.layerBounds.some((b, i) => !(0, import_core17.equals)(b, oldRenderInfo.layerBounds[i])) || renderInfo.allLayersLoaded !== oldRenderInfo.allLayersLoaded || renderInfo.layers.some((layer) => layer.props.transitions);
...
var _CollisionFilterExtension = class extends import_core19.LayerExtension {
  getShaders() {
...
var shader_module_default3 = {
  name: "mask",
  dependencies: [import_core20.project],
  vs: vs4,
  fs: fs3,
  inject: inject5,
  ...
  return device.createFramebuffer({
    ...
    colorAttachments: [
      device.createTexture({
        ...opts.float && {
          format: device.info.type === "webgl2" ? "rgba32float" : "rgba8unorm",
          type: import_constants4.GL.FLOAT
        },
        mipmaps: false,
        sampler: {
          minFilter: "linear",
...
var TerrainPass = class extends import_core23._LayersPass {
  ...
  renderHeightMap(heightMap, opts) {
    ...
    this.device.withParametersWebGL({
      clearColor: [0, 0, 0, 0],
      blend: true,
      blendFunc: [import_constants5.GL.ONE, import_constants5.GL.ONE],
      blendEquation: import_constants5.GL.MAX,
      depthTest: false
    }, () => this.render({
      ...opts,
      target,
...
var TerrainEffect = class {
  ...
  initialize(device) {
    ...
    (0, import_core25.getShaderAssembler)().addDefaultModule(terrainModule);
  }
  ...
  getModuleParameters(layer) {
    ...
    return {
      heightMap: (_a = this.heightMap) == null ? void 0 : _a.getRenderFramebuffer(),
      heightMapBounds: (_b = this.heightMap) == null ? void 0 : _b.bounds,
