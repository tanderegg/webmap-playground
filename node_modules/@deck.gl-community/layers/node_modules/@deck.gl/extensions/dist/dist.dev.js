...
`;
  ...
  // ../../node_modules/@luma.gl/constants/dist/constants-enum.js
...
`;
  function supportsFloatTarget(device) {
    const gl = device.gl;
    return Boolean(
      gl.getExtension("EXT_float_blend") && (gl.getExtension("EXT_color_buffer_float") || gl.getExtension("WEBGL_color_buffer_float"))
    );
  }
    ...
    initializeState(context, extension) {
      ...
      if (attributeManager) {
        attributeManager.add({
          filterValues: {
            size: filterSize,
            type: fp642 ? GLEnum.DOUBLE : GLEnum.FLOAT,
  ...
  // ../../node_modules/@luma.gl/shadertools/dist/lib/glsl-utils/highlight.js
  var glsl = (x) => `${x}`;
...
`,
  ...
  var MaskPass = class extends import_core14._LayersPass {
    ...
    render(options) {
      ...
      return this.device.withParametersWebGL(
        {
          blend: true,
          blendFunc: [GLEnum.ZERO, GLEnum.ONE],
          blendEquation: GLEnum.FUNC_SUBTRACT,
          colorMask,
          depthTest: false
        },
  ...
  function makeViewport(opts) {
    ...
    return isGeospatial ? new import_core15.WebMercatorViewport({
      id: viewport.id,
      x: border,
      y: border,
      width,
      ...
      zoom,
      orthographic: true
    }) : new import_core15.OrthographicViewport({
      id: viewport.id,
      x: border,
      y: border,
      width,
  ...
  var MaskEffect = class {
    ...
    preRender(device, { layers, layerFilter, viewports, onViewportActive, views, isPicking }) {
      ...
      if (viewport.resolution !== void 0) {
        import_core16.log.warn("MaskExtension is not supported in GlobeView")();
        return { didRender };
      }
    ...
    _sortMaskChannels(maskLayers) {
      ...
      for (const layer of maskLayers) {
        ...
        if (!channelInfo) {
          if (++channelCount > 4) {
            import_core16.log.warn("Too many mask layers. The max supported is 4")();
            continue;
          }
          channelInfo = {
            id,
  ...
  var CollisionFilterEffect = class {
    ...
    _render(renderInfo, {
      ...
      const needsRender = viewportChanged || renderInfo === oldRenderInfo || !(0, import_core19._deepEqual)(oldRenderInfo.layers, renderInfo.layers, 1) || renderInfo.layerBounds.some((b, i) => !equals(b, oldRenderInfo.layerBounds[i])) || renderInfo.allLayersLoaded !== oldRenderInfo.allLayersLoaded || renderInfo.layers.some((layer) => layer.props.transitions);
  ...
  var CollisionFilterExtension = class extends import_core20.LayerExtension {
    getShaders() {
...
`
  ...
  var shader_module_default3 = {
    name: "mask",
    dependencies: [import_core21.project],
    vs: vs4,
    fs: fs3,
    inject: inject5,
...
if ((terrain_mode == TERRAIN_MODE_USE_COVER) || (terrain_mode == TERRAIN_MODE_USE_COVER_ONLY)) {
  ...
  var TerrainEffect = class {
    ...
    initialize(device) {
      ...
      (0, import_core26.getShaderAssembler)().addDefaultModule(terrainModule);
    }
